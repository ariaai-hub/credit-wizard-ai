...
export default async function DocumentsPage() {
  ...
  const documentQueue = workspace.queue.filter((client) => ["INTAKE_RECEIVED", "DOCS_PENDING"].includes(client.lifecycleStage));
    ...
    <main className="app-frame px-6 py-8 md:px-10 md:py-10">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
        <header className="public-surface p-8 md:p-10">
          ...
          <h1 className="mt-4 text-4xl font-semibold tracking-[-0.05em] text-white md:text-5xl">Document collection</h1>
          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-300">
            Use this view to see which client files are still waiting on reports or intake documents.
          </p>
        </header>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="public-surface-soft p-5">
            <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-sky-200">Total clients</div>
            <div className="mt-3 text-3xl font-semibold text-white">{workspace.intakeOverview.totalClients}</div>
          </div>
          <div className="public-surface-soft p-5">
            <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-sky-200">Ready for strategy</div>
            <div className="mt-3 text-3xl font-semibold text-white">{workspace.readyForStrategy}</div>
          </div>
          <div className="public-surface-soft p-5">
            <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-sky-200">Mail queued</div>
            <div className="mt-3 text-3xl font-semibold text-white">{workspace.mailQueued}</div>
          </div>
        </section>
        ...
        <section className="public-surface p-6 md:p-8">
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="lux-label">Pending files</div>
              <h2 className="mt-3 text-2xl font-semibold tracking-[-0.04em] text-white">Records still waiting on documents</h2>
            </div>
          ...
          {documentQueue.length === 0 ? (
            <div className="mt-6 public-surface-soft p-6 text-sm text-slate-300">No files are waiting on documents right now.</div>
          ) : (
            <div className="mt-6 grid gap-3">
              {documentQueue.map((client) => (
